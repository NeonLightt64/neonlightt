...
exports.run = async (client, message, args) => {
 ...
 if (db.has(`log_${message.guild.id}`) === false) return message.reply('Mod log kanalı ayarlanmamış');
  ...
  if (message.mentions.users.size < 1) return message.reply('Lütfen atmak istediğiniz kullanıcıyı etiketleyin');
  if (reason.length < 1) return message.reply('Atma sebebinizi giriniz');
