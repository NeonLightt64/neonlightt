...
exports.run = async (client, message, params, args) => {
  ...
  if (!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send('<:hata:746320500359430184> Hoşgeldin kanalı ayarlamak için `Yönetici` yetkisine sahip olman gerek.')
  let hgkanali = message.mentions.channels.first();
  if (!hgkanali) return message.channel.send('<:hata:746320500359430184> Hoşgeldin kanalı ayarlamak için bir kanal etiketlemeniz gerekli. `!!gkanal #kanal`')
