...
exports.run = async (client, message, args) => {
 ...
 message.channel.send(mcbody);
    ...
    if (!args[0]) return message.channel.send('Lütfen bilgisini almak istedediğiniz işlemi yazın; işlemler ``kafa``,``skin``,``vücut``,``avatar``,``sunucu`` şeklinde olmalıdır; ``/minecraft kafa Shannara``');
    if (args[0] > 8) return message.channel.send('Lütfen bilgisini almak istedediğiniz işlemi yazın; işlemler ``kafa``,``skin``,``vücut``,``avatar``,``sunucu`` şeklinde olmalıdır; ``/minecraft kafa xDave12``');
