...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "bmgG7OnkkR2bemKppto3z_Y_V0Xc-P-o",
    callbackURL: `https://nl-neonlight.glitch.me/callback`,
    domain: `https://nl-neonlight.glitch.me/`
  };
  
  console.log('Panel ayarlandı')
...
//Tovbe tovbe kufurlere bak 
 ...
 const renderTemplate = (res, req, template, data = {}) => {
  ...
  app.get("/callback", passport.authenticate("discord", { failureRedirect: "/autherror" }), async (req, res) => {
    ...
    client.channels.get("779012840781512734").send(`**${client.users.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
...
res.redirect("/panel/"+req.params.guildID+"/genel");
  ...
  app.get("/ekle", (req, res) => {
   ...
   app.get("/seviye", (req, res) => {
    res.redirect(`https://nl-neonlight.glitch.me/kullanici/${req.params.kullaniciID}/yonet`);
  });
